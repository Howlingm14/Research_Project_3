...
anti_exploitation = np.ones((grid_dx, grid_dy))
...
class Env(gym.Env):
    ...
    def __init__(self):
        ...
        #self.texts = []
        global targid
        targid = p.loadURDF (os.path.join ("drive/My Drive/ColabNotebooks/5qlearning/urdfs/cf2x.urdf"),[9.5, -9.5, 3], [0, 0, 0, 1])#, useFixedBase=True, flags=p.URDF_USE_SELF_COLLISION)
    ...
    def coords_to_state(self):
        ...
        x = x_pos-3
        y = y_pos+9
        #print(anti_exploitation[x+1, y])
        #print(str(anti_exploitation[x+1, y]))
        A=str(0)
        B=str(0)
        C=str(0)
        D=str(0)
        if x>=0 and y>=0 and y<=8:
            A=str(int(anti_exploitation[y, x-1]))
        if y>=0 and x>=0 and x<=17:
            B=str(int(anti_exploitation[y-1, x]))
        if x<=17 and y>=0 and y<=8:
            C=str(int(anti_exploitation[y, x+1]))
        if y<=8 and x>=0 and x<=17:
            D=str(int(anti_exploitation[y-1, x]))
    ...
    def reset(self, x, y):
        ...
        #self.update()
        #time.sleep(0.5)
        p.resetBasePositionAndOrientation(p.getBodyUniqueId(targid), [x+3, y-9, 3], [0, 0, 0, 1])
    ...
    def step(self, action):
        ...
        x=int(next_state[0])-3
        y=int(next_state[1])+9
        
        if x<0 or y<0 or x>17 or y>8:
            reward=-100
            next_state = "Error"
        else:
            A=str(0)
        ...
        X_pos = x_pos-3
        Y_pos = y_pos+9
        if X_pos<0 or Y_pos<0 or X_pos>17 or Y_pos>8:
            reward=-100
        else:
            #global anti_exploitation
            reward = g[Y_pos, X_pos]*100
            #print(reward, Y_pos, X_pos)
            #print(X_pos, Y_pos, g[X_pos, Y_pos])
            g[Y_pos, X_pos] = 0
            anti_exploitation[Y_pos, X_pos] = 0
            #print("x", X_pos,"y", Y_pos)
            #print(g)
            #print(anti_exploitation)
            #print(g)
